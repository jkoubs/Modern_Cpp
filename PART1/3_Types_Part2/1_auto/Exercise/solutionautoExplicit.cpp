...
int main() {
  ...
  std::map<int, std::string> myMap = {{1, std::string("one")},
                                      {2, std::string("two")}};
  ...
  std::future<std::string> futureLambda =
      std::async([](const std::string &s) { return std::string("Hello ") + s; },
                 std::string("lambda function."));
  ...
  std::tuple<std::string, int, double, bool, char> tup =
      std::make_tuple(std::string("second"), 4, 1.1, true, 'a');
}
